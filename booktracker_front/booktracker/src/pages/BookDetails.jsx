...
export default function BookDetails() {
   ...
   const fetchBookDetails = async () => {
      try {
         ...
         if (id.length > 10) {
            console.log("String");
            res = await axios.get(`${baseUrl}/books/details/${id}`);
         } else {
            console.log("Long");
            res = await axios.get(`${baseUrl}/books/${id}`);
         }
   ...
   const handleAddToFavorites = async () => {
      ...
      try {
         await axios.post(`${baseUrl}/books/${book.idVolume}`);
         const res = await axios.get(`${baseUrl}/books/details/${book.idVolume}`);
         console.log(res.data.id);
         await axios.post(`${baseUrl}/users/${userId}/books/${res.data.id}`);
