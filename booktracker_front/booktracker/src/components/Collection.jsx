...
export default function Collection() {
    ...
    const fetchFavoriteBooks = async () => {
        ...
        try {
            ...
            // Récupérer les IDs Google Books depuis l'API
            const favoritesResponse = await axios.get(`${baseUrl}/users/${userId}/books`);
            const googleBookIds = favoritesResponse.data; // Liste de strings
            
            console.log("✅ IDs des favoris:", googleBookIds);

            if (!googleBookIds || googleBookIds.length === 0) {
                setBooks([]);
                setLoading(false);
                return;
            }

            // 2. Pour chaque ID, récupérer les détails du livre et sa review
            const booksWithDetails = await Promise.all(
                googleBookIds.map(async (bookIdVolume) => {
                    try {
                        // Récupérer les détails du livre avec l'idVolume
                        const bookResponse = await axios.get(`${baseUrl}/books/details/${bookIdVolume}`);
                        const bookData = bookResponse.data;

                        // Récupérer la review depuis l'API
                        let userReview = null;
                        try {
                            const reviewResponse = await axios.get(`${baseUrl}/google-book-reviews/${userId}/${bookIdVolume}`);
                            userReview = reviewResponse.data;
                        } catch (reviewError) {
                            console.log(`ℹ️ Pas de review pour le livre ${bookIdVolume}`);
                        }
                        ...
                        console.error(`❌ Erreur chargement livre ${bookIdVolume}:`, bookError);
                        return null;
                    }
                })
            );
    ...
    const handleSaveReview = async () => {
        ...
        try {
            const bookIdVolume = selectedBook.idVolume || selectedBook.id;
            
            const reviewData = {
                userId: parseInt(userId),
                googleBookId: bookIdVolume,
            ...
            if (selectedBook.userReview) {
                // Mise à jour
                await axios.put(`${baseUrl}/google-book-reviews/${userId}/${bookIdVolume}`, reviewData);
    ...
    const handleDeleteConfirm = async () => {
        ...
        try {
            ...
            // Retirer des favoris
            await axios.delete(`${baseUrl}/users/${userId}/books/${bookIdVolume}`);
            
            // Supprimer aussi la review
            if (selectedBook.userReview) {
                try {
                    await axios.delete(`${baseUrl}/google-book-reviews/${userId}/${bookIdVolume}`);
                } catch (reviewError) {
